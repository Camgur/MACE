...
for structure, string in zip(compiled, strings):
    ...
    # print(string, 'Li-O angle difference: ', bond_diff, '\n')
...
for structure, string in zip(compiled, strings):
    ...
    analyse = analysis.Analysis(exp, build_neighbor_list(exp))
    analyse2 = analysis.Analysis(structure, build_neighbor_list(structure))
    bonds = analyse.get_angles('O', 'Al', 'O', unique=True)
    bonds2 = analyse2.get_angles('O', 'Al', 'O', unique=True)
...
for structure, string in zip(compiled, strings):
    ...
    bonds = analyse.get_angles('Cl', 'Li', 'Cl', unique=True)
    bonds2 = analyse2.get_angles('Cl', 'Li', 'Cl', unique=True)
...
for structure, string in zip(compiled, strings):
    ...
    analyse = analysis.Analysis(exp, build_neighbor_list(exp))
    analyse2 = analysis.Analysis(structure, build_neighbor_list(structure))
    bonds = analyse.get_angles('Cl', 'Al', 'Cl', unique=True)
    bonds2 = analyse2.get_angles('Cl', 'Al', 'Cl', unique=True)
...
df.to_excel('MACE/optimise/LAC_35275/li-cl_angles.xlsx', index=True)

df = pd.DataFrame(compilation2, index=strings)
df.to_excel('MACE/optimise/LAC_35275/al-cl_angles.xlsx', index=True)
...
for structure, string in zip(compiled[:5], strings[:5]):
    ...
    analyse2 = analysis.Analysis(structure, build_neighbor_list(structure))
    bonds = analyse.get_angles('O', 'Li', 'O', unique=True)
    bonds2 = analyse.get_angles('O', 'Li', 'O', unique=True)
...
for structure, string in zip(compiled[:5], strings[:5]):
    ...
    analyse2 = analysis.Analysis(structure, build_neighbor_list(structure))
    bonds = analyse.get_angles('O', 'Al', 'O', unique=True)
    bonds2 = analyse.get_angles('O', 'Al', 'O', unique=True)
...
df.to_excel('MACE/optimise/LAC_35275/dftli-cl_angles.xlsx', index=True)

df = pd.DataFrame(compilation2, index=strings[:5])
df.to_excel('MACE/optimise/LAC_35275/dftal-cl_angles.xlsx', index=True)
'''