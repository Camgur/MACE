...
for structure, string in zip(compiled, strings):
    ...
    bonds = analyse.get_angles('O', 'Li', 'O', unique=True)
    bonds2 = analyse2.get_angles('O', 'Li', 'O', unique=True)
...
for structure, string in zip(compiled, strings):
    ...
    analyse = analysis.Analysis(exp, build_neighbor_list(exp))
    analyse2 = analysis.Analysis(structure, build_neighbor_list(structure))
    bonds = analyse.get_angles('O', 'Zr', 'O', unique=True)
    bonds2 = analyse2.get_angles('O', 'Zr', 'O', unique=True)
...
for structure, string in zip(compiled, strings):
    ...
    analyse = analysis.Analysis(exp, build_neighbor_list(exp))
    analyse2 = analysis.Analysis(structure, build_neighbor_list(structure))
    bonds = analyse.get_angles('O', 'P', 'O', unique=True)
    bonds2 = analyse2.get_angles('O', 'P', 'O', unique=True)
...
compilation3 = np.array(compilation3)

df = pd.DataFrame(compilation[0:6], index=strings)
df.to_excel('MACE/optimise/LZPO_91112/li-o_angles.xlsx', index=True)

df = pd.DataFrame(compilation2, index=strings)
df.to_excel('MACE/optimise/LZPO_91112/zr-o_angles.xlsx', index=True)

df = pd.DataFrame(compilation3, index=strings)
df.to_excel('MACE/optimise/LZPO_91112/p-o_angles.xlsx', index=True)
...
df = pd.DataFrame(compilation, index=strings[:5])
df.to_excel('MACE/optimise/LZPO_91112/dftli-o_angles.xlsx', index=True)

df = pd.DataFrame(compilation2, index=strings[:5])
df.to_excel('MACE/optimise/LZPO_91112/dftzr-o_angles.xlsx', index=True)

df = pd.DataFrame(compilation3, index=strings[:5])
df.to_excel('MACE/optimise/LZPO_91112/dftp-o_angles.xlsx', index=True)
'''